...
Meteor.methods({
    ...
    'startGame': function(roomId) {
        ...
        if (gameRoom.userId === Meteor.userId()) {
            ...
            var multTw = [
                0,7,14,0+7*15,7+7*15,14+7*15,0+14*15,7+14*15,14+14*15
            ];
            var numTiles = 15*15;
            var tiles = [];
    ...
    'makeMove': function(roomId, tilePlacements) {
        ...
        var words = [];
        if (longWord.length > 1) words.push(longWord);
        ...
        for (var wi = 0; wi < words.length; wi++) {
            if (!isValidWord(words[wi])) invalidWords.push(words[wi]);
        }
...
function getWordFromLineDefn(mainLine, tilePlacements, tiles, rack) {
    ...
    while (varyingIdx <= mainLine[1][mainLine[2]]) {
        ...
        if (placeIdx === -1 && tiles[tileId].letter === false) {
            ...
            word += tilePlacements[placeIdx][1];
        } else if (tiles[tileId].letter !== false) {
            word += tiles[tileId].letter;
        }
        varyingIdx++;
    }
    return word;
