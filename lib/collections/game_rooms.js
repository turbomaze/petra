...
Meteor.methods({
    ...
    'makeMove': function(roomId, tilePlacements) {
        ...
        for (var ai = 0; ai < tilePlacements.length; ai++) {
            ...
            rack[oldIdx].score = newLetter===false?false:LETTER_PTS[newLetter];
        }
        ...
        if (noMoreTiles) { //one of the ending conditions
...
function getWordFromLineDefn(mainLine, tilePlacements, tiles, rack) {
    ...
    while (varyingIdx <= mainLine[1][mainLine[2]]) {
        ...
        if (placeIdx === -1 && tiles[tileId].letter === false) {
            ...
            word += letter;
            var letterPts = LETTER_PTS[letter];
            ...
            word += letter;
            points += LETTER_PTS[letter];
        }
        varyingIdx++;
    }
    return [word, points*wordMult];
}
